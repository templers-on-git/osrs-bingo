...
1. **Search when adding items to a set** — `dev.js`'s "Add to set" control (Item Sets section) is still a plain flat `<select>` listing every item in the bank with no search, the same problem the tile-creation item/set picker in `login.html` already had and was fixed for (checkboxes + live search, see the Progress-tab/dual-panel-picker work). Needs the same treatment once there are many items — small, self-contained fix.
